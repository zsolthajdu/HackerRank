...
if len( descends ) == 0 :
	...
	if 1==len(descends[0]) and 1==len(descends[1]):
		...
		if (values[index1] > values[index2-1]) and ( (index2== finalElem) or ( values[index1] < values[index2+1]) ) :
			if values[index2] < values[index1+1] and (index1==0 or (values[index1-1] < values[index2])):
				print( "yes" )
...
if len( descends ) == 1 :
	...
	if (firstInd == 0 or (values[firstInd-1] < values[ lastInd ] )) and ( (lastInd == len(values)-1) or (values[firstInd] < values[ lastInd+1 ] ) ):
		print( "yes" )
